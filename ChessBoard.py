...
def _findCharucoboardCorners(img,board_index,board_dict):
    # define charucoboard\
    ARUCO_DICT=getattr(cv2.aruco, board_dict['dict'], None)
    SQUARES_VERTICALLY=board_dict['row']+1
    SQUARES_HORIZONTALLY=board_dict['col']+1
    SQUARE_LENGTH=board_dict['square_size']/1000
    MARKER_LENGTH=board_dict['marker_size']/1000
...
def findChessboardCorners(img, pointData, board_dict,is_charu,is_fisheye,num_board,type,old_k,dist):
    ...
    # Find the charucoboard corners
        ...
        for board_index in range(num_board):
            ...
            ret, corners, chids = _findCharucoboardCorners(gray,board_index,board_dict)  ##return coordinates&chids of corners of board
            if ret:
...
def detect_chessboard(imgPath, outPath, type, board_dict, gridSize, ext,is_charu,is_fisheye,num_board,old_k,dist):
    """
    ...
    show = findChessboardCorners(img, pointData, board_dict,is_charu,is_fisheye,num_board,type,old_k,dist) #Show is a picture with blue corner points attached with id.
